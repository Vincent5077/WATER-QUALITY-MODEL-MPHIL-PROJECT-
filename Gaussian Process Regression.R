...
library(tidyverse)
library(corrplot)
library(kernlab) # Needed for gaussprRadial
...
for (param in water_params) {
  ...
  # Train Gaussian Process Regression model (Radial basis kernel)
  model_gpr <- train(formula, 
                     data = data, 
  ...
  preds_log <- predict(model_gpr, newdata = data)
  obs_log <- data[[param]]
...
write.csv(summary_by_param, 'GPR_Summary.csv')
...
# Save metrics to CSV
write.csv(all_metrics, "D:/PROJECTS/Ankobra Raster/Random Forest/metrics_GPR.csv", row.names = FALSE)
