...
for (param in water_params) {
  ...
  preds_log <- predict(model_svm, newdata = data)
  obs_log <- data[[param]]
...
write.csv(summary_by_param, 'SVM_Summary.csv')

# Print metrics
print(all_metrics)

# Save metrics to CSV
write.csv(all_metrics, "D:/PROJECTS/Ankobra Raster/Random Forest/metrics_SVM.csv", row.names = FALSE)
